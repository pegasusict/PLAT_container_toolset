...
if ! options=$(getopt -o hm:f: -l help,metadata:,fstree:,name:,path:,rootfs:,mapped-uid:,mapped-gid: -- "$@")
then
	usage
	exit 1
fi
...
relevant_file() {
	local FILE_PATH	;	FILE_PATH="${LOCAL_TEMP}/$1"
	if [ -e "${FILE_PATH}-${MODE}" ]
	then
		FILE_PATH="${FILE_PATH}-${MODE}"
	fi
	if [ -e "${FILE_PATH}.${COMPAT_LEVEL}" ]
	then
...
if ! tar Jxf "${LXC_CONFIG}" -C "${LOCAL_TEMP}"
then
...
tar --anchored ${EXCLUDES} --numeric-owner -xpJf "${LXC_FSTREE}" -C \
"${LXC_ROOTFS}"
