...
download_file() {
	if ! wget_wrapper -T 30 -q "https://${DOWNLOAD_SERVER}/$1" -O \
	"$2" >/dev/null 2>&1
	then
		...
		then
			...
			then
				...
				echo "ERROR: Failed to download http://${DOWNLOAD_SERVER}/$1" \
				1>&2
				exit 1
			fi
		...
		then
			...
			echo -e "\t The file was instead downloaded over HTTP." 1>&2
...
gpg_setup() {
	...
	for _ in $(seq 3)
	do
		if gpg --keyserver "${DOWNLOAD_KEYSERVER}" --recv-keys \
		"${DOWNLOAD_KEYID}" >/dev/null 2>&1
		then
			success=1
			break
		fi
...
gpg_validate() {
	if [ "${DOWNLOAD_VALIDATE}" = "false" ]
	then
		if [ "${DOWNLOAD_SHOW_GPG_WARNING}" = "true" ]
		then
...
relevant_file() {
	...
	if [ -e "${FILE_PATH}-${DOWNLOAD_MODE}" ]
	then
		FILE_PATH="${FILE_PATH}-${DOWNLOAD_MODE}"
	fi
	if [ -e "${FILE_PATH}.${DOWNLOAD_COMPAT_LEVEL}" ]
	then
...
if ! options=$(getopt -o d:r:a:hl -l dist:,release:,arch:,help,list,variant:,\
			 server:,keyid:,keyserver:,no-validate,flush-cache,force-cache,\
...
if [ -z "${LXC_NAME}" ] || [ -z "${LXC_PATH}" ] || [ -z "${LXC_ROOTFS}" ]
then
	if [ "${DOWNLOAD_LIST_IMAGES}" != "true" ]
	then
		echo "ERROR: Please pass the name, path, and rootfs for the container" \
		1>&2
		exit 1
	fi
fi
...
then
	if [ "${USERNS}" = "yes" ]
	then
		if [ -z "${LXC_MAPPED_UID}" ] || [ "${LXC_MAPPED_UID}" = "-1" ]
		then
...
then
	...
	"${DOWNLOAD_TEMP}/index.asc" noexit
	then
		download_file "${DOWNLOAD_INDEX_PATH}" "${DOWNLOAD_TEMP}/index" normal
		download_sig "${DOWNLOAD_INDEX_PATH}.asc" "${DOWNLOAD_TEMP}/index.asc" \
		normal
	fi
	...
	do
		[ -n "${DOWNLOAD_DIST}" ] && [ "$f1" != "${DOWNLOAD_DIST}" ] && \
		 continue
		[ -n "${DOWNLOAD_RELEASE}" ] && [ "$f2" != "${DOWNLOAD_RELEASE}" ] && \
		 continue
		[ -n "${DOWNLOAD_ARCH}" ] && [ "$f3" != "${DOWNLOAD_ARCH}" ] && \
		 continue
		[ -n "${DOWNLOAD_VARIANT}" ] && [ "$f4" != "${DOWNLOAD_VARIANT}" ] && \
		 continue
		[ -z "${f5}" ] || [ -z "${f6}" ] && continue
		printf "%s\t%s\t%s\t%s\t%s\n" "${f1}" "${f2}" "${f3}" "${f4}" "${f5}"
...
# Setup the cache
if [ "${DOWNLOAD_TARGET}" = "system" ]
then
	LXC_CACHE_BASE="${LOCALSTATEDIR}/cache/lxc/"
else
	LXC_CACHE_BASE="${HOME}/.cache/lxc/"
fi
# Allow the setting of the LXC_CACHE_PATH with the usage of environment
#+ variables.
...
if [ -d "${LXC_CACHE_PATH}" ]
then
	if [ "${DOWNLOAD_FLUSH_CACHE}" = "true" ]
	then
		echo "Flushing the cache..."
		rm -Rf "${LXC_CACHE_PATH}"
	elif [ "${DOWNLOAD_FORCE_CACHE}" = "true" ]
	then
		DOWNLOAD_USE_CACHE="true"
	else
		DOWNLOAD_USE_CACHE="true"
		if [ -e "$(relevant_file expiry)" ]
		then
			if [ "$(cat "$(relevant_file expiry)")" -lt "$(date +%s)" ]
			then
...
then
	...
	echo "Downloading the image index"
	if ! download_file "${DOWNLOAD_INDEX_PATH}.${DOWNLOAD_COMPAT_LEVEL}" \
	"${DOWNLOAD_TEMP}/index" noexit || ! download_sig \
	"${DOWNLOAD_INDEX_PATH}.${DOWNLOAD_COMPAT_LEVEL}.asc" \
	"${DOWNLOAD_TEMP}/index.asc" noexit
	then
		download_file "${DOWNLOAD_INDEX_PATH}" "${DOWNLOAD_TEMP}/index" normal
		download_sig "${DOWNLOAD_INDEX_PATH}.asc" "${DOWNLOAD_TEMP}/index.asc" \
	...
	[ "$(cat "${LXC_CACHE_PATH}/build_id")" = "${DOWNLOAD_BUILD}" ]
	then
		...
		if ! tar Jxf "${DOWNLOAD_TEMP}/meta.tar.xz" -C "${LXC_CACHE_PATH}"
		then
		...
		if [ -n "${LXC_MAPPED_UID}" ] && [ "${LXC_MAPPED_UID}" != "-1" ]
		then
			...
			chown -R "${LXC_MAPPED_UID}" \
			"${LXC_CACHE_BASE}" >/dev/null 2>&1 || :
		fi
		if [ -n "${LXC_MAPPED_GID}" ] && [ "${LXC_MAPPED_GID}" != "-1" ]
		then
			chgrp -R "${LXC_MAPPED_GID}" \
			"${LXC_CACHE_BASE}" >/dev/null 2>&1 || :
		fi
...
#+ CentOS / Oracle
if [ -f "${LXC_ROOTFS}/etc/init/tty.conf" ]
then
...
if [ -n "${LXC_MAPPED_UID}" ] && [ "${LXC_MAPPED_UID}" != "-1" ]
then
	chown "${LXC_MAPPED_UID}" "${LXC_PATH}/config" \
	"${LXC_PATH}/fstab" >/dev/null 2>&1 || :
fi

if [ -n "${LXC_MAPPED_GID}" ] && [ "${LXC_MAPPED_GID}" != "-1" ]
then
	chgrp "${LXC_MAPPED_GID}" "${LXC_PATH}/config" \
	"${LXC_PATH}/fstab" >/dev/null 2>&1 || :
fi

if [ -e "$(relevant_file create-message)" ]
then
	echo ""
	echo "---"
